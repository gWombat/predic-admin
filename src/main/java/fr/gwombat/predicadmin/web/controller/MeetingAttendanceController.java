...
public class MeetingAttendanceController {
    ...
    @PostMapping
    public String saveOrUpdateMeetingAttendance(@ModelAttribute("attendance") @Valid MeetingAttendanceForm attendanceForm, BindingResult result, RedirectAttributes redirectAttributes) {
        if (result.hasErrors())
            return "attendances";
        ...
        try {
            ...
            logger.info(meetingAttendance + " " + e.getMessage());
        } catch (Exception e) {
            message = new DangerAlertMessage();
            message.setLabelCode("page.meeting.attendance.already.exists");
            logger.error(String.format("Error saving attendance [%s]", meetingAttendance), e);
        }
